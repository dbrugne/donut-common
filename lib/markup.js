...
module.exports = {
  ...
  // search for #donut/, #test-xd/
  rawGroupMentionPattern: /\s*(#([-a-z0-9_]{3,20})\/)/ig,
  ...
  parser: function (linkify) {
    return _.bind(function (string, enhanceCallback, finalCallback) {
      ...
      var match;
      var key;
      while ((match = this.rawRoomMentionPattern.exec(string)) !== null && (!match [ 3 ] || (match [ 3 ] && match [ 4 ]))) {
        key = keyGenerator();
        bag.rooms.push({
          key: key,
          match: match[ 1 ],
      ...
      while ((match = this.rawGroupMentionPattern.exec(string)) !== null) {
        key = keyGenerator();
        bag.groups.push({
          key: key,
          match: match[ 1 ],
  ...
  _find: function (string) {
    ...
    while ((markup = this.markupPattern.exec(string)) !== null) {
      var element = this._extractElement(markup);
      markups.push(element);
    }
    return markups;
  },

  _extractElement: function (markup) {
    console.log(markup);
    var element = {
      match: markup[ 0 ]
    };
  ...
  toObject: function (string) {
    this.markupPattern.lastIndex = 0; // required because regex is statefull. @see http://stackoverflow.com/questions/11477415/why-does-javascripts-regex-exec-not-always-return-the-same-value
    var markup = this.markupPattern.exec(string);
    if (!markup) {
      return;
    }
    return this._extractElement(markup);
  },
